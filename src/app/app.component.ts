...
export class AppComponent implements AfterViewInit {
    ...
    switchView = (viewType: 'grid' | 'list') => {
        const {gridButton, listButton, productsAreaWrapper} = this;

        if (!gridButton || !listButton || !productsAreaWrapper) {
            console.error("One or more required elements are not found.");
            return;
        }

        const activeButton = viewType === 'grid' ? gridButton : listButton;
        const inactiveButton = viewType === 'grid' ? listButton : gridButton;

        this.setActiveButton(activeButton, inactiveButton);
